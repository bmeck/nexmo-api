...
NexmoClient.prototype.getPricing = function getPricing(countryCode, callback) {
  if (!countryCode) {
    throw new Error('Invalid countryCode');
  }
  request.get({
    url: 'https://rest.nexmo.com/account/get-balance/'+this.key+'/'+this.secret+'/'+countryCode,
    headers: {
      'Accept': 'application/json'
    }
  }, function handleBalanceResponse(error, response, body) {
  ...
  switch (typeof options) {
    ...
    case 'object':
      ...
      if (options_or_newSecret.moCallBackUrl) {
        params.moCallBackUrl = options_or_newSecret.moCallBackUrl;
      }
      if (options_or_newSecret.drCallBackUrl) {
        params.drCallBackUrl = options_or_newSecret.drCallBackUrl;
      }
      break;
  }
...
NexmoClient.prototype.searchNumbers = function searchNumbers(options_or_countryCode, callback) {
  ...
  switch (typeof options_or_countryCode) {
    case 'string':
      ...
      break;
    case 'object':
      countryCode = options_or_countryCode.countryCode;
  ...
  if (!countryCode) {
    throw new Error('countryCode required');
  }
  request.get({
